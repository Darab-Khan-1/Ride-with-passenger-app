...
class TripController extends GetxController with GetSingleTickerProviderStateMixin {
  final rxRequestStatus = Status.LOADING.obs;
  RxString error = ''.obs;
  void setRxRequestStatus(Status _value) => rxRequestStatus.value = _value;
  void setError(String _value) => error.value = _value;
  RefreshController refreshHomeController = RefreshController();
  RefreshController refreshMyTripsController = RefreshController();
  TabController? tabController;

  final allTrips = AllTripModel().obs;
  final myTrips = AllTripModel().obs;
  ...
  Future<dynamic> getAllMyTrips() async{

    try {
      isloading.value = true;
      dynamic response = await _apiService.getApi(AppUrl.getAllMyTripApi);
      if(response['status_code'] == 200){
        isloading.value = false;
        myTrips.value = AllTripModel.fromJson(response);
        myTrips.refresh();
      }
      else if(response['status_code'] == 401){
        isloading.value = false;
  ...
  Future<dynamic> acceptTrip( int tripId, String status) async{
    try {
      isloading.value = true;
      update();
      ...
      dynamic response = await _apiService.postApi(data, AppUrl.acceptTripApi);
      if(response['status_code'] == 200){
